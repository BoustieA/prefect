...
def test_api_predict():
    ...
    try:
        ...
        assert data["prediction"] in [
            0,
            1,
        ], f"Prediction should be 0 or 1, got {data['prediction']}"
...
def test_complete_workflow():
    ...
    try:
        ...
        if pred2_data["model_version"] != retrain_data["model_version"]:
            print(
                f"   ⚠️ Model version mismatch: expected {retrain_data['model_version']}, got {pred2_data['model_version']}"
            )
...
def main():
    ...
    if success_rate >= 80:
        ...
        if DISCORD_WEBHOOK_URL:
            send_discord_embed(
                f"Validation globale terminée avec succès! {passed}/{total} tests passés ({success_rate:.1f}%)"
            )
    else:
        ...
        if DISCORD_WEBHOOK_URL:
            send_discord_embed(
                f"Validation globale échouée. {passed}/{total} tests passés ({success_rate:.1f}%)",
                "Échec",
            )
